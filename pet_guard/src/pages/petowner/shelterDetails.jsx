...
import { MapPin, Phone, Info, DoorOpen, CheckCircle, X } from "lucide-react";
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {shelter.documents.map((doc) => (
                      <img
                        ...
                        src={`${IMAGE_URL}/${doc}`}
                        alt="Document"
                        className="w-full h-40 object-cover rounded-2xl border border-white shadow-sm hover:shadow-md transition-shadow cursor-pointer"
                        onClick={() => setSelectedDocument(`${IMAGE_URL}/${doc}`)}
                      />
                    ))}
                  </div>
                </div>
