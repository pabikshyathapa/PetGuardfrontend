...
const InfoBox = ({ label, value, editMode, onChange, suffix = "", fieldName = "" }) => {
  const handleChange = (e) => {
    let newValue = e.target.value;
    
    // Apply validation based on field name
    if (fieldName === 'type' || fieldName === 'breed' || fieldName === 'gender') {
      newValue = validateTextOnly(newValue);
    } else if (fieldName === 'age' || fieldName === 'weight') {
      newValue = validateNumbersOnly(newValue);
    }
...
export default function PetProfilePage() {
  ...
  useEffect(() => {
    ...
    if (pets.length === 0) {
      ...
      setPreviewUrl({}); // Clear preview URLs when no pets
      return;
    }
  ...
  const handleRemovePet = async () => {
    ...
    try {
      ...
      // Clear preview URL for deleted pet
      setPreviewUrl((prev) => {
        const newPreview = { ...prev };
        delete newPreview[deletedId];
        return newPreview;
      });
      
      const remaining = pets.filter((p) => p._id !== deletedId);
      if (remaining.length > 0) {
        setSelectedPet(remaining[0]);
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (selectedPet && selectedPet._id && selectedPet._id !== "new") {
        ...
        updatedPetId = newPet._id;
      }

      setEditMode(false);

      const updatedPets = await fetchPets();
      const updatedPet = updatedPets.find((p) => p._id === updatedPetId);
      if (updatedPet) {
        setSelectedPet(updatedPet);
        handleSelectPet(updatedPet);
        // Clear ALL preview URLs to prevent showing wrong photos
        setPreviewUrl({});
      }
    } catch (err) {
  ...
  // Validation handler for emergency contact fields
  const handleEmergencyContactChange = (field, value) => {
    let newValue = value;
    
    if (field === 'emergencyContactName') {
      newValue = validateTextOnly(value);
    } else if (field === 'emergencyContactPhone') {
      newValue = validateNumbersOnly(value);
    }
      ...
      <main style={styles.mainContent}>
        ...
        <div style={styles.actionBar}>
          <button onClick={() => { 
            ...
            setEditMode(true); 
            setFormData(initialForm);
            setPreviewUrl({}); // Clear preview URLs when adding new pet
            ...
            <SwiperSlide key={pet._id}>
              <div style={styles.card}>
                ...
                <div style={styles.infoGrid}>
                  <InfoBox 
                    label="Type" 
                    value={editMode ? formData.type : pet.type} 
                    editMode={editMode} 
                  ...
                  <InfoBox 
                    label="Breed" 
                    value={editMode ? formData.breed : pet.breed} 
                    editMode={editMode} 
                  ...
                  <InfoBox 
                    label="Gender" 
                    value={editMode ? formData.gender : pet.gender} 
                    editMode={editMode} 
                  ...
                  <InfoBox 
                    label="Age" 
                    value={editMode ? formData.age : pet.age} 
                    suffix=" Years" 
                    editMode={editMode} 
                  ...
                  <InfoBox 
                    label="Weight" 
                    value={editMode ? formData.weight : pet.weight} 
                    suffix=" kg" 
                    editMode={editMode} 
                ...
                <div style={styles.sectionGrid}>
                  ...
                  <div style={styles.sectionBox}>
                    <h3 style={styles.sectionTitle}>Characteristics & Care Requirements</h3>
                    <div style={styles.sectionContentEdit}>
                      {editMode ? (
                ...
                <div style={styles.emergencySection}>
                  ...
                  <div style={styles.ownerGrid}>
                    <OwnerField 
                      ...
                      editMode={editMode} 
                      onChange={(e) => handleEmergencyContactChange('emergencyContactName', e.target.value)} 
                    />
                    <OwnerField 
                      label="Phone" 
                      ...
                      editMode={editMode} 
                      onChange={(e) => handleEmergencyContactChange('emergencyContactPhone', e.target.value)} 
                    />
                  </div>
                </div>
