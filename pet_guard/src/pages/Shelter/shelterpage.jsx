...
<div className="mb-5">
  ...
  <input
    type="text" // Changed to text for better manual control
    disabled={!editMode}
    value={data.pricePerDay || ""}
    onChange={(e) => {
      const value = e.target.value;
      // Regex: allows only digits (0-9). Remove the '?' if you don't want decimals.
      // If you want decimals, use: /^\d*\.?\d*$/
      if (value === "" || /^\d+$/.test(value)) {
        setData((prev) => ({ ...prev, pricePerDay: value }));
      } else {
        // Optional: Trigger a toast if they type a letter
